...
"""
Director + DreamerV2.5
- no free nats for kl in beta-vae -> --goal_kl_free 0.0
...
- .*\.layers: 4
- .*\.units: 512
- no symlog: --encoder.symlog_inputs False
- no symlog_disc -> --decoder.vector_dist mse
- no symlog_and_twohot -> --reward_head.dist mse --critic.dist mse
- no SILU -> .*\.act: elu
- discount 0.999 -> 0.99 -> --horizon 100
"""
  ...
  def abstract_traj(self, traj):
    ...
    for key, x in list(traj.items()):
      if key.startswith('reward_'):
        ...
        x = (reshape(jnp.concatenate([0 * x[:1], x], 0)) * w).sum(1)[1:]
      elif key == 'cont':
        x = reshape(x).prod(1)
      elif key == 'weights':
        x = reshape(x)[:, 0]
