...
class ResizeImage(base.Wrapper):
  ...
  def __init__(self, env, size=(64, 64), keep_original=False):
    super().__init__(env)
    self._size = size
    self._keys = [
        k for k, v in env.obs_space.items()
        if len(v.shape) > 1 and v.shape[:2] != size]
    self._keys_for_original = [f"fullsize_{k}" for k in self._keys] if keep_original else []
  ...
  def obs_space(self):
    ...
    for i, key_original in enumerate(self._keys_for_original):
      spaces[key_original] = spaces[self._keys[i]]
    for key in self._keys:
  ...
  def step(self, action):
    obs = self.env.step(action)
    for i, key_original in enumerate(self._keys_for_original):
      obs[key_original] = obs[self._keys[i]]
    for key in self._keys:
