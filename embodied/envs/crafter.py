...
class Crafter(embodied.Env):
  ...
  def __init__(self, task, size=(64, 64), outdir=None, seed=None):
    assert task in ('reward', 'noreward', 'healthreward')
    self._reward_type = task
    if task == 'healthreward':
      task = 'reward'
    import crafter
  ...
  def _obs(
    ...
    log_achievements = {
        ...
        for k in self._achievements}
    if self._reward_type == 'healthreward':
      # zero out rewards except when losing or gaining health
      if reward > 0.2:
        reward = reward - 1
    return dict(
        image=image,
