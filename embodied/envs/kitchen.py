...
class Kitchen(embodied.envs.from_gym.FromGym):
  ...
  def __init__(self, task, size=(64, 64), **kwargs):
    ...
    if 'MUJOCO_EGL_DEVICE_ID' not in os.environ:
      os.environ['MUJOCO_EGL_DEVICE_ID'] = os.environ.get('CUDA_VISIBLE_DEVICES', '0')

    import d4rl
    import embodied.envs.kitchen_in_order
  ...
  def step(self, action):
    ...
    for task in TASKS:
        self.solved_tasks[task] = self.solved_tasks[task] or task not in self._env.tasks_to_complete
