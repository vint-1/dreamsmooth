...
class Generic:
  ...
  def add(self, step, worker=0, load=False):
    ...
    if self.online:
      ...
      if len(stream) >= self.length and ( # TODO Make this work with reward smoothing????
    ...
    if len(stream) < self.length or self.counters[worker] < self.stride:
      return
    self.counters[worker] = 0 # TODO Add reward smoothing right here!!
    key = embodied.uuid()
    seq = tuple(stream)
    # import ipdb; ipdb.set_trace()
    # embodied.smooth_rewards(seq, 3) # TODO FIX THIS SHIT
    if load:
