...
def train(agent, env, replay, logger, args):
  ...
  should_log = embodied.when.Clock(args.log_every) # logs every y seconds
  should_save = embodied.when.Clock(args.save_every) # saves every x seconds
  step = logger.step
  ...
  def per_episode(ep):
    ...
    for key in args.log_keys_video:
      if key in ep:
        metrics.video(f'stats/policy_{key}', (ep[key], ep['reward']))
  ...
  driver.on_step(lambda tran, _: step.increment()) # increment step counter
